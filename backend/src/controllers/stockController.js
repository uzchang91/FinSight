...
exports.getAllStocks = async (req, res) => {
  try {
    const [rows] = await db.promise().query("SELECT * FROM stocks ORDER BY stock_name ASC");
...
exports.getStockChart = async (req, res) => {
  try {
    ...
    const [rows] = await db.promise().query("SELECT * FROM stocks WHERE stock_code = ?", [symbol]);
    if (rows.length === 0) return fail(res, "해당 종목을 찾을 수 없습니다.", null, 404);
    ...
    return success(res, "종목 차트 조회 성공", {
      symbol: stock.stock_code, name: stock.stock_name, market: stock.market_type, range, interval, prices,
    });
  } catch (err) {
...
exports.getStockPrice = async (req, res) => {
  try {
    ...
    return success(res, "종목 현재가 조회 성공", {
      symbol: stock.stock_code, name: stock.stock_name, market: stock.market_type,
      price: quote.regularMarketPrice, change: quote.regularMarketChange,
      changeRate: quote.regularMarketChangePercent, currency: quote.currency, marketState: quote.marketState,
    });
  } catch (err) {
